...
var metadataElements = [
  ...
  {
    name: "User-Item Ratio",
    info: "user-item-ratio-info",
    key: "userItemRatio",
    fixed: 2,
    better: "range",
    rangeDescription: [
  ...
  {
    name: "Max Rating/User",
    description: "Highest Number of Rating By Single User",
    key: "highestNumberOfRatingBySingleUser",
  },
  {
    name: "Min Rating/User",
    description: "Lowest Number of Rating By Single User",
    key: "lowestNumberOfRatingBySingleUser",
  },
  {
    name: "Max Rating/Item",
    description: "Highest Number of Rating On Single Item",
    key: "highestNumberOfRatingOnSingleItem",
  },
  {
    name: "Min Rating/Item",
    description: "Lowest Number of Rating On Single Item",
    key: "lowestNumberOfRatingOnSingleItem",
    fixed: 2,
  },
  {
    name: "Mean Interactions per User",
    description: "Mean Number of Ratings By User",
    info: "mean-ratings-per-user-info",
  ...
  {
    name: "Mean Interactions per Item",
    description: "Mean Number of Ratings On Item",
    info: "mean-ratings-per-item-info",
...
export async function initialize(queryOptions) {
  ...
  datasetComparisonFilterElement = document.getElementById(
  ...
  if (queryOptions && queryOptions.datasets) {
    initialSelectedDatasetIds = queryOptions.datasets
      .split(" ")
      .map((id) => Number(id));
  } else {
  ...
  document
    .getElementById("compare-database-share-btn")
    .addEventListener("click", shareDatabaseComparison);
  document
    .getElementById("dataset-export-csv-btn")
    .addEventListener("click", exportDatasetTableCsv);
  currentSortKey = "name";
  currentSortDirection = "asc";
  compareDatasets();
  updateFilterHeader(
...
function compareDatasets() {
  const selectedDatasetIds = Array.from(
    datasetComparisonFilterElement.querySelectorAll(
  ...
  selectedDatasets.forEach((dataset) => {
    ...
    const cells = metadataElements
      ...
      .map((meta) => {
        ...
        return `<td data-key="${meta.key}">${formatValue(
          value,
          decimals
        )}</td>`;
      });
...
function toggleAllColumns() {
  ...
  selectedColumns = shouldCheck ? metadataElements.map((col) => col.key) : [];
...
function shareDatabaseComparison() {
  const datasetIds = selectedDatasets.map((ds) =>
    typeof ds === "object" ? ds.id : ds
  );
  const columnIds = selectedColumns.map((col) =>
    typeof col === "object" ? col.id : col
  );
...
function generateUserItemRatioTooltip(datasets) {
  ...
  const levels = [
    {
      description: "Extremely item-heavy",
      risk: "High",
      cause: "Too many items with too few users",
      bias: "Strong long-tail bias",
      coldStart: "High (items)",
    },
    {
      description: "Very item-heavy",
      risk: "Medium",
      cause: "Items start getting ignored",
      bias: "Long-tail bias",
      coldStart: "Medium (items)",
    },
    {
      description: "Slightly item-heavy",
      risk: "Low",
      cause: "Good coverage of items and users",
      bias: "Low",
      coldStart: "Low",
    },
    {
      ...
      risk: "Low",
      cause: "Optimal learning conditions",
      bias: "Minimal",
      coldStart: "Low",
    },
    {
      description: "Slightly user-heavy",
      risk: "Medium",
      cause: "Fewer items, users converge on same content",
      bias: "Popularity bias",
      coldStart: "Medium (users)",
    },
    {
      description: "Very user-heavy",
      risk: "High",
      cause: "Too few items for many users",
      bias: "Heavy popularity bias",
      coldStart: "High (users)",
    },
    {
      description: "Extremely user-heavy",
      risk: "High",
      cause: "Severe lack of items",
      bias: "Strong popularity bias",
      coldStart: "High (users)",
    },
  ];
  ...
  ranges.forEach((r) => {
    const row = document.createElement("tr");
    [r.label, r.description, r.risk, r.cause, r.bias, r.coldStart].forEach(
      (value) => {
        const td = document.createElement("td");
...
function generateMeanRatingsPerUserTooltip(datasets) {
  ...
  const thead = document.createElement("thead");
  const headRow = document.createElement("tr");
  ["Range", "Risk Level", "Cause", "Bias Risk", "Cold-Start Risk"].forEach(
    (title) => {
      const th = document.createElement("th");
      th.textContent = title;
      th.style = "border: 1px solid #ccc; padding: 6px; background: #e9ecef;";
  ...
  const levels = [
    {
      risk: "High",
      cause: "Users do not interact enough to learn preferences",
      bias: "Strong long-tail bias",
      coldStart: "High (users)",
    },
    {
      risk: "Medium",
      cause: "Sparse signals",
      bias: "Slight popularity bias",
      coldStart: "Medium (users)",
    },
    { risk: "Low", cause: "Balanced", bias: "Low", coldStart: "Low" },
    {
      risk: "Medium",
      cause: "Power users dominate, skewed model",
      bias: "Activity bias",
      coldStart: "Low",
    },
    {
      risk: "High",
      cause: "Overfitting to active users",
      bias: "User bias",
      coldStart: "High",
    },
  ];
...
function generateMeanRatingsPerItemTooltip(datasets) {
  ...
  const levels = [
    {
      risk: "High",
      cause: "Items too underexposed",
      bias: "Long-tail bias",
      coldStart: "High (items)",
    },
    {
      risk: "Medium",
      cause: "Weak signals for most items",
      bias: "Slight popularity bias",
      coldStart: "Medium",
    },
    { risk: "Low", cause: "Balanced", bias: "Low", coldStart: "Low" },
    {
      risk: "Medium",
      cause: "Few dominant items",
      bias: "Popularity bias",
      coldStart: "Low",
    },
    {
      risk: "High",
      cause: "Head dominates, tail ignored",
      bias: "Strong popularity bias",
      coldStart: "High",
    },
  ];
